# RSA - 80

```
A Daedalus Corp spy sent an RSA-encrypted message. 
We got their key data, but we're not very good at math. Can you decrypt it? Here's the data. 
Note that the flag is not a number but a number decoded as ASCII text.
```

Straight up old RSA challenge. 

In the handout folder, there are two text files:
* ciphertext.txt
* key_data.txt

```
Ciphertext:

Key_data.txt:
N = 
e = 65537
p = 0xc315d99cf91a018dafba850237935b2d981e82b02d994f94db0a1ae40d1fc7ab9799286ac68d620f1102ef515b348807060e6caec5320e3dceb25a0b98356399
q = 0xe90bbb3d4f51311f0b7669abd04e4cc48687ad0e168e7183a9de3ff9fd2d2a3a50303a5109457bd45f0abe1c5750edfaff1ad87c13eed45e1b4bd2366b49d97f
d = 
```

If you wonder what RSA is, you should read this: http://en.wikipedia.org/wiki/RSA_(cryptosystem)

Since they give us everything, all we need to do is just decrypt the ciphertext and get the flag.
I wrote a simple python script.

```
from isis import *

N = 
e = 65537
p = 0xc315d99cf91a018dafba850237935b2d981e82b02d994f94db0a1ae40d1fc7ab9799286ac68d620f1102ef515b348807060e6caec5320e3dceb25a0b98356399
q = 0xe90bbb3d4f51311f0b7669abd04e4cc48687ad0e168e7183a9de3ff9fd2d2a3a50303a5109457bd45f0abe1c5750edfaff1ad87c13eed45e1b4bd2366b49d97f
d = 
c = 

RSA(c,d,N)

anny@tests-MacBook-Pro:~/Desktop/CTF/tools$ python test2.py
[*]Decrypted Flag: Congratulations on decrypting an RSA message! Your flag is modular_arithmetics_not_so_bad_after_all
```

```
Flag:modular_arithmetics_not_so_bad_after_all
```


